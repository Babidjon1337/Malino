...
async def callback_bonus_url(callback: CallbackQuery, state: FSMContext):
    ...
    await callback.message.edit_text(
        ...
        "Пригласи друга по ссылке — и получи новый расклад без платы.\n"
...
async def callback_sleep(callback: CallbackQuery, state: FSMContext):
    ...
    if await rq.check_subscription(callback.from_user.id):
        ...
        await callback.message.answer(
            "🌙 <b>Сонник доступен только обладателю подписки</b> — чтобы расшифровать сны глубже, тебе нужно стать Premium пользователем.\n\n"
            "✨ Выбери свой путь:\n"
            "🔹 <b>Получи ещё одно бесплатное гадание</b> — пригласи друга и раскрой новую возможность.\n"
            "🔹 <b>Получи безграничные возможности</b> — теперь ты можешь гадать сколько угодно раз, каждый день, без ограничений.\n\n"
            f"💡 Сейчас тебе доступно {await rq.caunt_taro(callback.from_user.id)} таро-гаданий.\n\n"
            "— Малина всегда с тобой ❤️",
...
async def callback_tarot(callback: CallbackQuery, state: FSMContext):
    ...
    if await rq.check_tarot(callback.from_user.id):
        ...
        await callback.message.answer(
            "🔮 <b>Твои бесплатные расклады закончились</b>, но Малина по-прежнему с тобой!\n\n"
            "✨ Выбери свой путь:\n"
            "🔹 <b>Получи ещё одно бесплатное гадание</b> — пригласи друга и раскрой новую возможность.\n"
            "🔹 <b>Получи безграничные возможности</b> — теперь ты можешь гадать сколько угодно раз, каждый день, без ограничений.\n\n"
            f"💡 Сейчас тебе доступно 0 таро-гаданий.\n\n"
            "— Малина всегда с тобой ❤️",
            reply_markup=kb.btn_attempts,
        )
...
async def message_tarot(message: Message, state: FSMContext):
    ...
    msg = await message.answer(
        "Выбери <b>3 карты</b>, которые говорят с твоей душой.\n"
        "✨ Выбранные карты появятся здесь. Когда закончишь, нажми «Трактовка»."
    )
...
async def webapp_tarot(
    ...
    try:
        ...
        await bot.edit_message_text(
            chat_id=user_id, message_id=message_id, text=response, parse_mode="HTML"
        )
...
async def callback_card_day(callback: CallbackQuery, state: FSMContext):
    ...
    if await rq.check_card_day(callback.from_user.id):
        ...
        if callback.data == "card_day_reminder":
            ...
            if await rq.check_subscription(callback.from_user.id):
                await callback.message.answer(
                    "💫 Не забывай, у Малины всегда есть для тебя:\n\n"
                    "🃏 <b>Расклад таро</b> — получи ответ на любой вопрос\n"
                    "🌙 <b>Сонник</b> — расшифруй послания своих снов\n\n"
                    "Твоя подписка открывает все возможности!",
                    reply_markup=kb.btn_reminder_subscription,
                )
            elif remaining_tarot > 0:
                await callback.message.answer(
                    f"💡 <b>У тебя ещё есть {remaining_tarot} таро-гаданий</b>\n\n"
                    "✨ Если есть вопрос - нажми на кнопку, и я раскрою тайны твоих карт",
                    reply_markup=kb.btn_tarot_from_reminder,
                )
            else:
                await callback.message.answer(
                    "🔮 Кстати, твои бесплатные таро-гадания закончились.\n"
                    "Хочешь, я расскажу, как получить ещё?",
                    reply_markup=kb.btn_more_info_from_reminder,
                )

    else:
        await callback.message.answer(
            "<b>🔮 Ты уже получил(а) свою Карту Дня сегодня.</b>\n"
            "Энергия этой карты ещё с тобой — прислушайся к её шепоту.\n\n"
            "✨ Следующая Карта Дня откроется для тебя завтра.\n"
            "Не спешите — Вселенная даёт ответы в своё время.\n\n"
            "Жди утром… или просто приходи, когда почувствуешь — она будет ждать. 💫\n\n"
            "— Малина всегда с тобой ❤️"
        )
...
async def callback_management_subscription(callback: CallbackQuery, state: FSMContext):
    ...
    if subscription and subscription.tariff != "VIP":
        ...
        await callback.message.edit_text(
            "<b>⏳ Твоя подписка закончилась</b> — и доступ к безлимиту приостановлен.\n\n"
            "Ты уже знаешь, как удобно гадать по Таро без ограничений, раскрывать сны в Соннике и получать персональные ответы каждый день.\n"
